...
const LandingPageScrollContainer: FunctionComponent<ScrollContainerProps> = ({
  ...
  return (
    <section className="col-9 bg-success scroll-container-size">
      <h2 className="text-start">{category}</h2>
      <div className=" overflow-x-scroll d-flex flex-row hstack gap-2 border border-secondary">
        {content.map((element, index) => (
          <Link
            to={`/services/${category}`}
            key={index}
            className="text-decoration-none w-100 service-card shadow-sm"
            onClick={handleClick}
          >
            <Card>
