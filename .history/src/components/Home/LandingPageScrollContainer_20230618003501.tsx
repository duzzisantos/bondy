...
const LandingPageScrollContainer: FunctionComponent<ScrollContainerProps> = ({
  ...
  return (
    <section className="col-9 scroll-container-size">
      ...
      <div className="overflow-x-scroll d-flex flex-row hstack gap-2 border border-secondary rounded-3 rounded-bottom-0">
        {content.map((element, index) => (
          <Link
            to={`/services/${category}`}
            key={index}
            className="text-decoration-none w-50 shadow-sm"
            onClick={handleClick}
          >
