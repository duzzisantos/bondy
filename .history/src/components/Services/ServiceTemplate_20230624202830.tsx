        ...
        .map((item) => (
          <div className="text-start vstack gap-3">
            ...
            <Card className="p-4 custom-color">
              ...
              <hr />
              <Card.Body className="d-flex flex-row vstack gap-3 overflow-x-auto">
                {Object.values(item.images).map((image: any, index) => (
                  <div key={index} className="col-sm-3 image-height">
                    <Card.Img src={`${image.link}`} alt="Fake image" />
                    <Card.Subtitle>{`Service ${index}`}</Card.Subtitle>
                  </div>
                ))}
              </Card.Body>
            </Card>
